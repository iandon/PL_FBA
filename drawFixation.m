...
function drawFixation(task)
    ...
    if  checkFix && (1 <= task.thistrial.thisseg < 11)  %(stimulus.FixationBreak(task.trialnum))  %If there is a fixn break, present 
    ...
    elseif ~task.thistrial.gotResponse || (task.thistrial.thisseg == 11)
    ...
    elseif (task.thistrial.thisseg == 9 && ~task.thistrial.gotResponse); %No fixation before response
    
    elseif ((task.thistrial.thisseg == 9 && task.thistrial.gotResponse) || (task.thistrial.thisseg == 10 && task.thistrial.gotResponse)) %Once there is a response, present fixation of a different color
    ...
    elseif (task.thistrial.thisseg == 10 && ~task.thistrial.gotResponse)  %If there is no response, present orange fixation
