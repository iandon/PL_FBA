...
n_repeats = 2; % n = 2 -> 72 trials per block (1*2*2*3*3 *2)
...
elseif (task.thistrial.thisseg == 1) % fixation
    ...
    %response cue
    stimulus.tmp.respCueOri=stimulus.randVars.targetOriAxis(task.thistrial.trialIndex); 
...
elseif (task.thistrial.thisseg == 2) % Pre Cue
    ...
    if ~stimulus.FixationBreakCurrent  || ~stimulus.EyeTrack
     drawPreCue(stimulus.tmp.respCueOri);
%       drawRespCue(stimulus.tmp.respCueOri)
    end
...
elseif (task.thistrial.thisseg == 8) % Resp Cue
    ...
    if stimulus.EyeTrack && ~stimulus.FixationBreakCurrent, fixCheck(myscreen,task); end
    if ~stimulus.FixationBreakCurrent  || ~stimulus.EyeTrack
        drawRespCue(stimulus.tmp.respCueOri); % has to be a positive integer
    end
